    ...
    public class Venta_Repository : IVenta_Repository
    {
        ...
        public async Task ActualizarStock(List<DetalleVenta> detalleVentas)
        {
            ...
            foreach(var detalle in detalleVentas)
            {
                ...
                if (producto == null)
                {
                    throw new KeyNotFoundException($"el producto con el ID: {detalle.idProducto} no existe en el invenatrio.");
                }

                if(producto.stock < detalle.cantidad)
                {
                    throw new InvalidOperationException($"No hay sufieciente stock para el producto: {producto.descripcion}. Stock disponible: {producto.stock}, cantidad solicitada: {detalle.cantidad}.");
                }
        ...
        public async Task<Venta> Create(Venta venta)
        {
            ...
            try
            {
                ...
                {
                    throw new InvalidOperationException("No se pudo generar el número de factura. Inténtelo nuevamente.");
                }
        ...
        {
            var detallesVenta = await _posContext.DetalleVenta
                .Where(dv => dv.idVenta == idVenta)
                .ToListAsync();

            if(detallesVenta.Count == 0)
            {
                throw new InvalidOperationException($"No se encontraron detalles de venta para el ID de venta: {idVenta}. Verifica que el ID es correcto.");
            }
